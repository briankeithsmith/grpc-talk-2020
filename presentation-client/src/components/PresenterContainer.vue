...
@Component({
  components: {
    FontAwesomeIcon,
    StartPresentingForm: () => import("./StartPresentingForm.vue")
  }
})
export default class PresentationContainer extends VueClass {
  ...
  @Prop({ default: true, type: Boolean }) public showSartPresenting!: boolean;

  public startPresentingModalActive = false;
  public transitionName: string = "fade";
  public talkName: string = "grpc";
  public startLoading: boolean = false;

  public get startingPresentation() {
  ...
  public async startPresentingSubmitted(password: string) {
    try {
      this.startLoading = true;
      const didStart = await store.dispatch.slides.startPresenting({
        password
      });
      if (didStart) {
        this.startPresentingModalActive = false;
      }
    } finally {
